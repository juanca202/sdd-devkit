...
| `archivedPath` | ruta relativa (ej. `docs/specs/archived/`) | Dónde el skill que produce cada artefacto lo archiva cuando el usuario lo pide con el modificador `archive` (`work-define`, `work-plan`, `requirement-refine`, `work-research`), espejando las subcarpetas de `changesPath`; `work-integrate`/`pr-create` no archivan. Reemplaza a `archivePath`. Ver el contrato de archivado en [references/artifacts.md](references/artifacts.md). |
...
| `workTreePath` | ruta (por defecto `.worktrees/`) | Dónde crear los worktrees, **relativa a la raíz del repositorio de código afectado**: el repo principal en un repo único o monorepo; **cada submódulo afectado** en multi-repo (un worktree por repositorio tocado). El repositorio de especificaciones nunca se clona en un worktree. Detalle en [`work-implement/references/worktrees.md`](skills/work-implement/references/worktrees.md). |
...
Los secretos nunca van en `settings.json`: credenciales del gestor de proyectos o de cualquier herramienta local viven en `.sdd-devkit/.env` (no versionado; `arch-init` lo deja en `.gitignore`).
