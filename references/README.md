...
| [`planning.md`](planning.md) | Política de definición de casos de prueba resuelta desde `.sdd-devkit/settings.json`: `testCases.createMode` decide si se pregunta, se invoca automáticamente o nunca se sugiere `test-define` al cerrar la planificación; `testCases.createDetailsMode`, si `test-define` entrevista o aplica valores por defecto | `work-define`, `work-plan` (`mode`) · `test-define` (`createDetailsMode`) |
| [`implementation.md`](implementation.md) | Política de implementación resuelta desde `.sdd-devkit/settings.json`: ritmo de confirmación por unidad, qué hacer con cambios sin commitear al iniciar, worktrees y su ubicación, concurrencia máxima, handoff de cierre, modo de archivado y ubicación del código bajo prueba (`scope`: `code` / `tests`, con detección de respaldo por `.env`) | `work-implement` · `work-integrate`, `pr-create` (`archiveMode`) |
