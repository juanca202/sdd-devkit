...
- **Los artefactos generados de las puertas son commiteables como cualquier otro cambio.** `docs/audits/*.md` y los `criteria-coverage.md` van con tipo `docs`; su **borrado** también (ver la tabla de tipos).

**Los renombrados masivos de documentación van con tipo `docs`.** El archivado de un `US-XXX`/`WI-XXX` es el caso canónico: `docs(specs): archivar US-042 tras integrarla`. No es `chore` — mueve documentación, y el diff, si git detecta el *rename*, es solo la ruta. Ver [`${PLUGIN_ROOT}/references/archive.md`](../../references/archive.md#contrato-para-el-resto-del-catálogo).
