...
> **Renombrados: exigir `-M` y no bloquear por un falso positivo del archivado.** El commit del archivado de un `US-XXX`/`WI-XXX` mueve una carpeta entera de documentación (`git mv` a `docs/specs/archived/`). Si git **detecta el rename**, el diff es solo la ruta y no hay línea añadida que inspeccionar. Si **no** lo detecta —umbral de similitud, o `diff.renames` desactivado en el repo—, el contenido íntegro de cada archivo movido aparece como líneas `+`, así que un `README.md` o un `coverage.md` que cite `token =` o `api_key` en un criterio de aceptación, en la salida de un test o en un ejemplo dispara el patrón. Ese hallazgo es **espurio**: nada nuevo entra en la historia, el archivo ya estaba versionado.
>
