...
>
> **Sin cambios en el código no se repite ningún check.** Si `.sdd-devkit/quality-check-run.json` está fresco (mismo `FINGERPRINT` canónico), todos los checks presentes en la caché se toman de ahí en **cualquier modo** — una caché escrita por una corrida completa guarda también tipado, linter, build y sonar (`checks[]`), así que **no se re-ejecuta nada**; solo se ejecutan los checks que la caché no traiga (p. ej. los estáticos, cuando la escribió una corrida `tests-only`). Solo `no-cache` o una petición explícita del usuario fuerzan la re-ejecución. Ver [Caché de corrida de pruebas](#caché-de-corrida-de-pruebas-compartida-con-coverage-verify).
>
...
- **El informe vive en `docs/audits/`, no en la carpeta de una US/WI**, porque la corrida es de la rama consolidada y puede abarcar varios trabajos. (En modo `tests-only` no hay informe: el único artefacto es `quality-check-run.json`.) Ver [Caché de corrida de pruebas](#caché-de-corrida-de-pruebas-compartida-con-coverage-verify).
...
| 8 | E2E | Condicional (Bloqueante si el estándar de testing la exige) | Se ejecuta sobre la aplicación ya compilada. **No es fija:** sin config e2e queda en `N/A` y **se omite del informe**, salvo que el estándar la declare (entonces es `SKIPPED` y sí se lista). |
| 9 | Análisis estático (Sonar) | Informativo | Nunca bloquea. |

El orden sigue la pirámide de tests (*rápido → lento*, *dependencias antes que consumidores*): estático (tipado/linter/arquitectura) → unit+coverage → suites configuradas → build → e2e → sonar. Las validaciones de arquitectura son **análisis estático determinista** —no tocan la red ni levantan servicios—, por eso van con el resto del estático y no con las suites de prueba. Una suite configurada que **requiera la aplicación compilada** (rendimiento, carga, accesibilidad sobre la app desplegada) se ejecuta después de build, junto a e2e. El fail-fast solo aplica al tipado, para evitar ruido en cascada. Justificación detallada en [`references/execution.md`](references/execution.md#paso-2--ejecutar-los-checks).
...
> **Los checks deterministas alimentan la caché de `quality-check-run.json`** —las dos fijas (`unit`, `coverage`), más `e2e` y las suites configuradas cuando existen, más `architecture` cuando el repo tiene runner— ver [Caché de corrida de pruebas](#caché-de-corrida-de-pruebas-compartida-con-coverage-verify). Sus consumidores son `coverage-verify` (las suites de prueba; **ignora `architecture`**, que no es cobertura funcional) y `arch-audit` (solo `architecture`). Una corrida completa guarda además tipado, linter, build y sonar en `checks[]`, cuyo único consumidor es este propio skill.
...
| `tests-only` | Ejecutar **solo los checks deterministas que alimentan `quality-check-run.json`** (las dos fijas —unit, coverage— más e2e y las suites configuradas, cuando existen, más las **validaciones de arquitectura** si el repo tiene runner; build solo si es prerrequisito de alguna de ellas); omitir tipado/linter/sonar. Las validaciones de arquitectura entran aquí **para que la caché escrita cubra siempre el conjunto vigente**: una corrida que las dejara fuera produciría una caché parcial, que no se escribe. Pensado como **objetivo de delegación de `coverage-verify`**: honra la caché de corrida de pruebas — si existe un `quality-check-run.json` **fresco** (fingerprint coincide, ver [Caché de corrida de pruebas](#caché-de-corrida-de-pruebas-compartida-con-coverage-verify)) **reutiliza** ese resultado sin re-ejecutar; si no, ejecuta y escribe/actualiza la caché (sin `checks[]`: los estáticos no corren en este modo). **Modo no interactivo:** devuelve los resultados por suite y la ruta de `quality-check-run.json` **sin** entrar al ciclo de corrección, **sin** emitir veredicto y **sin** escribir `quality-check.md` — su única salida es `quality-check-run.json`. Si hay suites en FAIL, se reportan como tales; corregirlas es decisión del flujo que invocó, no de esta corrida. |
...
5. **Registro y salida:** escribir siempre el informe en `docs/audits/quality-check.md` y —**solo si la corrida ejecutó el conjunto de pruebas completo** (las dos fijas, e2e y todas las suites configuradas que apliquen)— la caché en `.sdd-devkit/quality-check-run.json` (creando los directorios si no existen), incluyendo en `checks[]` los checks estáticos ejecutados (tipado, linter, build, sonar) cuando la corrida fue completa, más un resumen en el chat. **Excepción `tests-only`:** no hay informe ni veredicto; el único artefacto es `quality-check-run.json`. **No** hacer commit/push/merge sin instrucción explícita.

> **Tras cualquier corrección, el código cambió: recalcular el fingerprint** (Paso 1) antes de escribir la caché. Escribir un `quality-check-run.json` con el fingerprint previo lo vuelve falso — afirmaría corresponder a un estado del código que ya no existe.
...
| Contexto | Qué hacer |
|----------|-----------|
| **Dentro de una implementación** — hay un **trabajo en curso** al que atribuir la rama: un artefacto del plugin (`US-XXX`, `WI-XXX`, o `FT-XXX`/`TC-XXX` sobre rama `test/`) **o un artefacto externo** (ticket, spec suelto) que el usuario o la rama señalen. **No se exige carpeta ni `progress.md`** (cierre vía `work-integrate` / `pr-create`) | Mostrar el reporte y **preguntar si se corrige**. Es el flujo normal del cierre: corregir es lo esperado, pero sigue requiriendo autorización. **Excepción — repositorio de pruebas externo** (`.sdd-devkit/settings.json` con `implementation.target: external`): una prueba en rojo **no se corrige ni se delega** — no hay código de producción en ese repo. Se reporta como **hallazgo** (comportamiento del sistema distinto al `TC-XXX`) y se remite a la regla de prueba en rojo de `work-implement/references/test-cases.md` § Modo externo; solo los checks propios del repo de pruebas (lint, tipado, build, configuración del runner) siguen siendo corregibles con autorización. |
...
runner), persiste el resultado en `.sdd-devkit/quality-check-run.json` —ruta
...
re-ejecuta. Una corrida completa guarda también sus checks estáticos (tipado, linter, build, sonar) en
`checks[]`, de modo que la siguiente corrida de este skill con el mismo fingerprint **no re-ejecuta
absolutamente nada** y re-emite informe y veredicto desde la caché. Cada consumidor lee **solo sus entradas** —`coverage-verify` las suites de prueba,
...
El orden recomendado en el cierre es `quality-check` → `code-review` → `coverage-verify`: los dos primeros porque revisar diseño sobre código que ni compila suele ser trabajo perdido; el tercero **después de este skill** para que reutilice el `quality-check-run.json` sin re-ejecutar pruebas. Es una recomendación del orquestador, no una dependencia dura, y el usuario puede pedir solo uno de los tres.
