...
- **Duplicado de ID:** si el usuario proporciona `SRS-XXX`, confirmar que esa carpeta no existe ya en `<changesPath>/requirements/`.
...
1. **Fijar el ID y nombre de carpeta**
   - Usar el `SRS-XXX` indicado por el usuario o inferir el siguiente libre listando carpetas `SRS-*` en `<changesPath>/requirements/`.
...
4. **Diseño de interfaz: wireframes (si aplica)** — bloque **UX** de la Ola 2 (ver [`interview.md`](interview.md)): las preguntas de este paso se formulan en lenguaje de experiencia (qué ve y hace el usuario, dónde y en qué contexto), nunca de implementación.
   ...
   - Si el resultado (inferido o preguntado) es **Sí**:
     ...
     3. **Resolver la capability de cada pantalla.** Los wireframes no viven en la carpeta del SRS sino en `docs/architecture/[capability]/wireframes/` — la misma carpeta de capability donde `design-define` guarda `models/`, `flows/` y `diagrams/` (ver [`${PLUGIN_ROOT}/references/artifacts.md`](../../../references/artifacts.md)). **Inferir primero** la capability a la que pertenece cada pantalla: por el alcance y los actores del SRS, y por las carpetas que ya existen en `docs/architecture/` (si hay una `facturacion/` y el requerimiento es de facturas, es esa; nunca crear una segunda carpeta para la misma capability con otro nombre). Pantallas de un mismo SRS pueden ir a capabilities distintas — cada una a la suya. **Preguntar solo si la inferencia queda ambigua** (ninguna carpeta existente encaja y el nombre nuevo no es evidente, o una pantalla podría pertenecer a dos), con la herramienta estructurada y las capabilities candidatas como opciones. Nombre de carpeta nueva: minúsculas, kebab-case, sin artículos (misma convención de `design-define`).
     4. **Generar el wireframe SVG de todas las pantallas de una vez**, sin pedir feedback entre una y otra: para cada una, un archivo `docs/architecture/[capability]/wireframes/WF-XXX-[pantalla-slug].svg` con un **mockup visual de baja/media fidelidad** — proporciones razonablemente realistas de cada región y componente, contenido de ejemplo, paleta en **escala de grises** para distinguir botones/campos/texto/contenedores, tipografía genérica — **sin colores de marca, tipografía real ni medidas pixel-perfect**, coherente con el tipo de solución ya definido. **Enlazarlo** (nunca pegar el código SVG) desde su documento hermano `WF-XXX-[pantalla-slug].md` (plantilla: [`${PLUGIN_ROOT}/skills/design-define/assets/wireframe-template.md`](../../design-define/assets/wireframe-template.md)). El `WF-XXX` es un id secuencial de 3 dígitos **por capability** — continuar la secuencia de los `WF-*` que ya existan en esa carpeta `wireframes/`, sin renumerar ninguno — y el slug se fija al crear (renombrar la pantalla después no renombra los archivos; el id es el contrato de enlace, igual que `MD-XXX`/`DG-XXX`). **En la misma pasada**, agregar la fila de cada wireframe a la tabla índice **«Wireframes»** del `README.md` de la capability (id, pantalla, tipo de solución, origen = enlace a este SRS, estado de revisión); si la carpeta de la capability no existe todavía, crearla con `README.md` a partir de [`capability-readme-template.md`](../../design-define/assets/capability-readme-template.md) con solo **Propósito**, **Wireframes** y **Observaciones** — el resto lo completará `design-define` cuando toque. Es la única escritura que este skill hace en `docs/architecture/`: nunca crear ni editar `models/`, `flows/`, `diagrams/` ni APIs desde aquí.
     5. **Presentar el lote completo en una sola respuesta**, no una por wireframe: listar el `WF-XXX` y el nombre de cada pantalla con el enlace a su documento (`docs/architecture/[capability]/wireframes/WF-XXX-[pantalla-slug].md`) y a su SVG (`…/WF-XXX-[pantalla-slug].svg`) para poder abrirlos y revisarlos con facilidad. El SVG se referencia por enlace, no se pega como código en la respuesta; si el entorno donde corre el agente renderiza imágenes en markdown, incluir además la imagen embebida (`![...](ruta)`) junto al enlace.
     6. **Preguntar una sola vez, con todos los wireframes ya generados y listados**, si hay cambios: `[Aprobar todos tal cual]` / `[Hay cambios que hacer]`. Si hay más de una pantalla y el usuario elige que hay cambios, usar **selección múltiple** para que indique en qué pantalla(s) — el propio paso lo justifica, no son mutuamente excluyentes (ver [`${PLUGIN_ROOT}/references/asking.md`](../../../references/asking.md)); con una sola pantalla, saltar directo al siguiente punto.
     ...
     8. **Volver a presentar el lote actualizado** (las pantallas que cambiaron, con su SVG embebido si el entorno lo renderiza, o enlazado en caso contrario) y repetir la pregunta de aprobación hasta `[Aprobar todos tal cual]` o hasta que el usuario indique que deja algo pendiente.
     9. **Registrar el resultado** en la tabla de la sección 11 del SRS: pantalla, capability, enlace al wireframe (`WF-XXX`), estado de revisión (`Pendiente` / `Revisado con cambios` / `Aprobado`) y un resumen de qué observaciones quedaron incorporadas a qué `FR-XXX`. Sincronizar el mismo estado en la cabecera del `WF-XXX-[pantalla-slug].md` y en su fila del índice «Wireframes» del README de la capability — las tres deben decir lo mismo.
...
5. **Verificación y trazabilidad de los requisitos** — el **criterio de verificación** de cada requisito se levanta con el rol dueño (PO para `FR-XXX`, ARCH para `NFR-XXX`) en su lenguaje: «¿cómo sabrás que esto quedó cumplido?» (ver etapa 09 en [`interview.md`](interview.md)).
   ...
   - Registrar todo en la tabla de la sección 12 del SRS (**Verificación y trazabilidad**): `ID`, `Origen`, `Depende de`, `Método de verificación`, `Criterio de verificación`. No duplicar el enunciado del requisito aquí — ya está en las secciones 6/7; esta tabla solo referencia el `ID`.
...
6. **Riesgos**
   ...
   - Registrar en la tabla de la sección 13 del SRS (**Riesgos**).
...
10. **Redactar el** `README.md` usando `assets/srs-template.md` como molde:
    ...
    - **Enlaces y archivos de apoyo:** enlazar cada archivo de `references/` guardado en el paso 2, la investigación de stack si se hizo, y la(s) carpeta(s) `docs/architecture/[capability]/wireframes/` si aplica.
...
11. **Validación final — Ola 3 (Validation)**, con el `README.md` ya redactado; detalle completo en [`interview.md`](interview.md#ola-3-en-detalle-paso-11-de-flowmd):
...
12. **Glosario** (si aplica): términos de dominio nuevos van a `docs/glossary.md`, igual criterio que en `work-define`.

13. **Cierre**
    - Si el SRS queda en **Draft**, resumir las lagunas agrupadas por sección (`FR-XXX`, `NFR-XXX`, Interfaces externas, Requisitos de datos, Cumplimiento normativo, Wireframes, Verificación y trazabilidad, Riesgos, Stack, Repositorios) y ofrecer, en la siguiente interacción, cerrar cada una con preguntas estructuradas.
    ...
    - **Repositorios sin harness:** por cada repositorio de la sección **Repositorios e implementación**, verificar si tiene `AGENTS.md` y `.sdd-devkit/settings.json` en su raíz — un repositorio marcado **Nuevo** nunca los tiene todavía; uno **Existente** se verifica si es accesible localmente, y si no lo es, preguntar directamente al usuario. Si **alguno** de los repositorios carece del harness, agregar **`/arch-init`** como opción adicional en la misma pregunta de cierre, junto a `/work-define` — no es excluyente ni bloquea el handoff, es una sugerencia más que el usuario puede tomar antes, después o en vez de continuar con `/work-define`.

---

## Flujo: Actualizar un SRS existente

1. **Identificar el archivo** — por ID, nombre-corto o título, buscándolo en `<changesPath>/requirements/`.
...
3. **Aplicar los cambios** solicitados. Reglas invariantes:
   ...
   - Si el usuario pide un cambio de UI sobre un wireframe ya aprobado, tratarlo como una nueva vuelta de revisión (paso 4 del flujo de creación): actualizar el wireframe (mismo `WF-XXX`, nunca uno nuevo para la misma pantalla), registrar la observación en su Historial de revisión, reflejar el impacto en los `FR-XXX` afectados y sincronizar el estado en la sección 11 y en el índice «Wireframes» de la capability antes de volver a marcarlo `Aprobado`.
...
- Wireframes, si los hay, en `docs/architecture/[capability]/wireframes/WF-XXX-[pantalla-slug].md` (con `design-define/assets/wireframe-template.md`) y su `WF-XXX-[pantalla-slug].svg` hermano enlazado, no pegado como código; capability inferida (o preguntada si era ambigua); fila de cada uno en el índice «Wireframes» del README de la capability; tabla en la sección Diseño de interfaz sincronizada con el estado real de cada uno
...
- Guardar los wireframes en `assets/` de la carpeta del SRS (o en cualquier ruta fuera de `docs/architecture/[capability]/wireframes/`), nombrarlos solo por el slug de la pantalla sin el `WF-XXX`, o crearlos sin su fila en el índice «Wireframes» del README de la capability — un wireframe huérfano no lo encuentra `work-define` ni `design-define`.
- Crear una segunda carpeta de capability con otro nombre cuando ya existe una equivalente en `docs/architecture/`, o crear/editar `models/`, `flows/`, `diagrams/` o APIs desde este skill: la única escritura permitida en `docs/architecture/` es `wireframes/` y su fila de índice.
...
- Preguntar por el equipo de desarrollo (integrantes, roles, contactos): no forma parte del SRS; se define fuera de este skill.
