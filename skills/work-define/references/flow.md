...
- **Confirmaciones de flujo (después de redactar o en cierre Draft):** **una pregunta por turno** cuando sea posible; si hay más de tres lagunas, encadenar tandas como en el paso 6 del flujo de creación.
...
- **Duplicado de ID:** si el usuario proporciona `US-XXX`, confirmar que esa carpeta no existe **ni en `<changesPath>/user-stories/` ni en `<archivedPath>/user-stories/`**. Un ID archivado sigue ocupado: reutilizarlo dejaría dos historias distintas bajo el mismo identificador y volvería ambiguo todo lo que las referencia (ramas, commits, work items del tracker). Ver [`${PLUGIN_ROOT}/references/archive.md`](../../../references/archive.md#contrato-para-el-resto-del-catálogo).
- **Solapamiento de alcance:** revisar los títulos y descripciones de otras US —las de `<changesPath>/user-stories/` **y las de `<archivedPath>/user-stories/`**— para detectar si el actor + valor + alcance ya está cubierto por una historia existente. Una US archivada es trabajo **ya entregado**: si el alcance coincide, redefinirlo es duplicarlo, y hay que decírselo al usuario.
...
- <razón concreta>
- [US-XXX: Título](<changesPath>/user-stories/US-XXX-nombre/README.md) — <razón del solapamiento, si aplica>
```
...
5. **Al redactar cada historia, heredar del SRS en lugar de volver a preguntar lo que ya resolvió:**
   ...
   - **Referencias:** si el SRS tiene wireframes aprobados en su sección 11 relevantes a esta historia, enlazarlos directamente por su ruta en la capability (`docs/architecture/[capability]/wireframes/WF-XXX-[pantalla-slug].md` y su `.svg`) en vez de preguntar por referencias de diseño ni generar wireframes nuevos (el paso 3 del flujo de creación no aplica) — la fila **Referencias de UI** del DoR puede marcarse `Cumple` de inmediato. Si la historia cubre una pantalla que el SRS no wireframeó, entonces sí aplica el paso 3 sobre esa pantalla, con origen = esta US. El enlace al propio SRS no va aquí: va en la línea `Requerimiento:` de la cabecera (bullet siguiente).
   ...
   - **Contexto/Observaciones:** si el SRS trae stack ya resuelto (sección 3), interfaces externas (sección 8), requisitos de datos (sección 9) o cumplimiento normativo (sección 10) relevantes a esta historia, resumirlos en **Contexto** o citarlos para justificar dimensiones de INVEST o filas del DoR — no se repiten como secciones nuevas, la US no duplica la estructura del SRS.
   - **Documentación técnica:** si algún `FR-XXX`/`NFR-XXX` de esta historia implica flujos, modelos de datos o APIs (a menudo señalado por las secciones 9-11 del SRS), aplica igual la delegación a `/design-define` del paso 4 del [flujo de creación](#flujo-crear-una-historia-nueva).
6. **Al terminar de crear todas las historias del lote, actualizar la tabla de la sección 15 (`Historias de usuario derivadas`) del `README.md` del SRS** — una fila por historia creada, con su `US-XXX`, título y los `FR-XXX` que cubre. Esta es la única escritura que este skill hace sobre un `SRS-XXX`; el resto del documento es de solo lectura para este flujo. Si no es posible editar el SRS, informar al usuario qué fila habría que agregar en vez de omitirlo en silencio.
...
1. **Fijar el ID y nombre de carpeta**
  - Usar el `US-XXX` indicado por el usuario o inferir el siguiente libre listando carpetas `US-*` **en `<changesPath>/user-stories/` y en `<archivedPath>/user-stories/`**, tomando el mayor de las dos: archivar no libera el número.
...
2. **Escribir el** `README.md` usando `assets/user-story-template.md` como molde:
  ...
  - **Referencias:** enlaces de diseño, wireframes `WF-XXX` de la capability (heredados, existentes o generados en el paso 3) y archivos en `assets/`; los archivos aportados no deben quedar solo en el chat. Si el requerimiento trae imágenes, enlaces a Figma o archivos `.md` con diagramas, wireframes o prototipos, **leerlos primero** para incorporarlos al contexto; ante lagunas, conflictos o falta de claridad detectados en ellos, trasladar esas dudas a la tanda de preguntas estructuradas antes de redactar (ver [Checklist antes de redactar](#checklist-antes-de-redactar)).
  - **Criterios de aceptación** (lista plana con ids `AC-XXX`):
    - Cada criterio usa id secuencial **AC-001**, **AC-002**, … único en el ámbito de la US. **El id es inmutable una vez publicado:** no se renumera al reordenar ni al eliminar criterios — se asigna el siguiente libre a los nuevos y se marca el eliminado como obsoleto en su propio enunciado. El id es un **contrato de enlace**: `test-define` lo cita verbatim en cada `TC-XXX`, la línea `Casos de prueba:` cuelga de él y todos los `criteria-coverage.md` lo cruzan literalmente; renumerar rompe esas tres cosas en silencio. Es la misma doctrina que aplica `design-define` a los ids de sus elementos.
...
3. **Wireframes** (si la US toca UI y no hereda wireframes)
  - **Detectar la necesidad:** la historia tiene interfaz propia (pantallas, formularios, paneles — `AC-XXX` de categoría *Interacción de usuario* o *Salidas del sistema*) y **no** existe ya un wireframe para esas pantallas: ni heredado de un `SRS-XXX` (ver [Flujo: Descomponer un SRS-XXX](#flujo-descomponer-un-srs-xxx-en-historias)), ni aportado por el usuario como diseño/Figma, ni presente ya en `docs/architecture/[capability]/wireframes/` (revisar el índice «Wireframes» del README de la capability antes de generar nada — si la pantalla ya tiene `WF-XXX`, enlazarlo, no duplicarlo). Si el usuario trae un diseño de alta fidelidad, no se genera wireframe: se enlaza el diseño. Inferir; preguntar solo si queda ambiguo si la US tiene UI propia.
  - **Dónde y cómo:** los wireframes son un elemento de la **capability**, no de la US — viven en `docs/architecture/[capability]/wireframes/WF-XXX-[pantalla-slug].md` + `WF-XXX-[pantalla-slug].svg` (SVG hermano, enlazado, nunca pegado como código), con la plantilla [`${PLUGIN_ROOT}/skills/design-define/assets/wireframe-template.md`](../../design-define/assets/wireframe-template.md) y **fila en la tabla índice «Wireframes» del `README.md` de la capability** (origen = enlace a esta US), en la misma pasada. Es el mismo estándar de `models/`/`diagrams/`: id secuencial de 3 dígitos por capability (continuar la secuencia de los `WF-*` existentes en esa carpeta, sin renumerar), slug fijado al crear, referencia por ruta de archivo sin ancla. **Capability:** inferirla del alcance/actor de la US y de las carpetas existentes en `docs/architecture/` (nunca crear una segunda carpeta para una capability equivalente); preguntar con la herramienta estructurada solo si es ambigua. Si la carpeta no existe, crearla con `README.md` a partir de [`capability-readme-template.md`](../../design-define/assets/capability-readme-template.md) con solo **Propósito**, **Wireframes** y **Observaciones**. **Es la única escritura permitida de este skill en `docs/architecture/`** — `models/`, `flows/`, `diagrams/` y APIs siguen siendo exclusivos de `design-define` (paso 4).
  - **Mismo mecanismo de lote que `/requirement-refine`** (paso 4 de su [`flow.md`](../../requirement-refine/references/flow.md)): tipo de solución (web / nativa / híbrida / escritorio, responsiva o no) inferido del contexto de la US y preguntado solo si es ambiguo — condiciona navegación y `viewBox`; identificar **todas** las pantallas de la US, generar **todos** los SVG de una vez (mockup en escala de grises, baja/media fidelidad, contenido de ejemplo, sin marca ni pixel-perfect), presentar el lote completo en **una sola respuesta** (`WF-XXX` + nombre + enlace al `.md` y al `.svg`, imagen embebida si el entorno la renderiza) y preguntar **una sola vez** `[Aprobar todos tal cual]` / `[Hay cambios que hacer]` (selección múltiple de pantallas si hay más de una). Cada cambio pedido actualiza el SVG, se registra en el **Historial de revisión** del `.md` y, si implica un criterio nuevo o modificado, **actualiza o crea el `AC-XXX` correspondiente** — nunca un cambio de wireframe sin reflejo en los criterios. Repetir hasta aprobar o hasta que el usuario deje algo pendiente.
  - **Enlazar y sincronizar:** cada `WF-XXX` va en **Referencias** de la US como `docs/architecture/[capability]/wireframes/WF-XXX-[pantalla-slug].md` (copiar la ruta tal cual, sin recomponerla desde el nombre de la pantalla); el estado de revisión se mantiene igual en la cabecera del `.md` del wireframe y en su fila del índice de la capability. Con todos los wireframes aprobados, la fila **Referencias de UI** del DoR pasa a `Cumple`; uno `Pendiente` la deja en `Parcial` y es una laguna más para el cierre Draft.
4. **Documentación técnica** (delegada a `design-define`)
  ...
  - **Enlazar las referencias devueltas:** el subagente responde con la lista de elementos creados/actualizados y su referencia ya formada (p. ej. `docs/architecture/facturacion/models/MD-001-factura.md`, `apis/API-001-facturas.md#post-invoices` para un endpoint concreto, `flows/FL-001-emision-factura.md` o `diagrams/DG-001-contexto.md`). Agregarlas a la sección **Referencias** del `README.md` de la US **copiándolas tal cual**: la referencia es la ruta del archivo del elemento —`MD-XXX-{slug}` / `API-XXX-{slug}` / `FL-XXX-{slug}` / `DG-XXX-{slug}`— más el ancla de operación `#<método-ruta sin versión>` cuando se cita un endpoint; los emite `design-define` de forma explícita. No recomponerlas a partir del nombre del elemento (`#api-001-crear-factura`, `README.md#api-001` o `models/factura.md` no existen) ni «mejorarlas». Si el subagente reporta lagunas técnicas pendientes, reflejarlas en Observaciones de la US.
  - **No integrarla en la descripción funcional** de la US. Además de Referencias, solo puede citarse desde las secciones INVEST u Observaciones del DoR para justificar complejidad, dependencias o restricciones técnicas que condicionan algún criterio (p. ej. *«Ver* `architecture/facturacion/apis/API-001-facturas.md#post-invoices` *— justifica la estimación de la dimensión E»*).
5. **Glosario** (si aplica)
...
## Flujo: Sincronizar una historia desde el gestor de proyectos (`sync`)

Solo con la integración con el gestor de proyectos **activada** y el modificador **`sync`** explícito (ver `SKILL.md` → Modificadores de invocación). El detalle de lectura y mapeo por proveedor vive en `references/<proveedor>.md` (p. ej. [`azure-devops.md`](azure-devops.md)); este flujo es el esqueleto común. `sync` **crea** la historia local si no existe y la **actualiza** si ya existe; en ambos casos el work item es el origen y el `README.md` la especificación.

1. **Resolver la entrada.** Uno o varios identificadores (`#4821`, `4821`, URL de work item, o un `US-<id>` local cuya cabecera tenga `Work Item (<Sistema>)`). Si alguno no se puede resolver a un `id` numérico, preguntar; no adivinar.
2. **Leer cada work item** vía MCP y **verificar que es una historia** (tabla de tipos de la referencia del proveedor). Un tipo distinto no se sincroniza: se remite al skill que corresponde (`work-plan`, `test-define`, `work-research`) o, si es un contenedor (Feature/Epic), se ofrece listar sus historias hijas. Sin MCP, degradar pidiendo el contenido pegado; el número de la US es el `id` indicado.
3. **Localizar la US local.** Buscar `US-<id>-*` y cabeceras con `Work Item (<Sistema>): [#<id>]` en la ruta activa **y** en el archivo. Si está archivada, parar y avisar (trabajo cerrado; desarchivar lo decide el usuario). Si no existe → **crear** (pasos 4-7). Si existe → **actualizar** (paso 8).
4. **Mapear** los campos del work item a la plantilla (título, Descripción COMO/QUIERO/PARA, Contexto, Criterios de aceptación, dependencias, complejidad, datos de gestión) siguiendo la referencia del proveedor. **No inventar nada que el work item no diga**; lo que la plantilla exige y el work item no trae es una laguna en Observaciones.
5. **Codificar los criterios.** Si ya traen identificador, usarlo verbatim; si no, asignar `AC-001…` en orden y anotarlo en Observaciones. Categoría entre paréntesis y vocabulario RFC 2119 solo cuando no cambien el sentido. Un criterio ambiguo o no verificable se conserva, se marca como laguna y deja la US en `Draft`. Esta codificación **no se escribe de vuelta** en el tracker.
6. **Preguntar solo lo que el tracker no tiene** — normalmente los **Repositorios** afectados (una sola tanda para todo el lote; omitir si es inferible o el repo es único).
7. **Crear** `changesPath/user-stories/US-<id>-[slug]/README.md` con el `id` del tracker como número (sin padding), `Work Item (<Sistema>): [#<id>](<url>)` en la cabecera, INVEST y DoR calculados, y `Estado` según los criterios de Ready del skill (el estado del tracker solo orienta; ver la referencia del proveedor). Un lote se crea en orden de dependencias, como en «Proponer varias historias».
8. **Actualizar** (US ya existente): leer el `README.md` completo y presentar **un diff por sección** —título, Descripción, Criterios de aceptación (emparejados por identificador cuando el tracker los codifica; por orden y texto cuando no), Contexto de gestión, dependencias— en una sola tanda, y aplicar únicamente lo que el usuario confirme con las reglas del flujo «Actualizar una historia»: los `AC-XXX` existentes son **inmutables** (un criterio nuevo toma el siguiente libre; uno que desapareció del tracker se marca `Obsolete`, no se borra); si cambia el enunciado de un criterio con `Casos de prueba:`, avisar de la desalineación y sugerir `test-define`; recalcular INVEST/DoR y el `Estado`. Sin cambios → decirlo y no tocar el archivo.
9. **Cerrar** con el mensaje explícito de sincronización — «Sincronizada US-<id> desde <Sistema> (#<id>): creada → <ruta>» o «…: actualizadas <secciones>» — y el handoff habitual: si quedó `Ready`, resolver `test-define` según `createMode`; si quedó `Draft`, ofrecer las preguntas que cierran las lagunas.

> **Nunca se escribe en el gestor de proyectos desde este flujo**: ni estado, ni criterios renumerados, ni título abreviado. Es dirección única tracker → repo.

---

## Flujo: Actualizar una historia existente

1. **Identificar el archivo** — por ID, nombre-corto o título. Si no aparece en `<changesPath>/user-stories/`, buscarlo bajo `<archivedPath>/user-stories/` antes de darlo por inexistente. **Si está archivado, parar y avisar:** la historia ya se cerró e integró, y modificarla exige desarchivarla primero —mover su carpeta de vuelta—, decisión que es del usuario. **Nunca** crear una carpeta nueva en la ruta activa por no haber encontrado la historia.
...
5. **Si la US queda en** `Estado: Draft` tras los cambios (sea porque ya lo estaba, sea porque los cambios la degradaron desde Ready), ofrecer al usuario las preguntas que cerrarían las lagunas residuales mediante la **herramienta de preguntas estructuradas**, aplicando las mismas reglas del paso 6 del flujo de creación (una pregunta por laguna, opciones cuando la respuesta admita categorías, máximo tres por bloque, encadenar tandas si hace falta). Si las respuestas completan los checklists de INVEST y DoR, promover a `Estado: Ready`. Si el usuario prefiere mantener Draft o salta preguntas, respetarlo y reflejar el residual en Observaciones.
...
- Si la US toca UI: wireframes `WF-XXX` en `docs/architecture/[capability]/wireframes/` (heredados del SRS, ya existentes, o generados en el paso 3 con lote único y una sola pregunta de aprobación), con fila en el índice «Wireframes» de la capability y enlazados desde Referencias por su ruta exacta; nunca en `assets/` de la US
...
**Ejemplo 0 — Sincronizar desde el gestor de proyectos**
- *Entrada:* «/work-define sync #4821» (integración con ADO activa, MCP disponible).
- *Comportamiento:* lee el work item, confirma que es `User Story`, comprueba que no existe `US-4821-*` ni una US con `Work Item (ADO): #4821`, mapea título/descripción/criterios, asigna `AC-001…` porque los criterios venían como viñetas, pregunta el repositorio afectado, crea `<changesPath>/user-stories/US-4821-<slug>/README.md` con `Work Item (ADO)` y cierra: «Sincronizada US-4821 desde ADO (#4821): creada». Ofrece `/test-define` según `createMode`. No toca el work item. Una segunda invocación sobre el mismo `#4821` no crea nada: muestra el diff por sección y aplica solo lo confirmado.

**Ejemplo 0b — `#id` sin modificador**
- *Entrada:* «US nueva para el #4821, que el cliente pueda pagar con tarjeta».
- *Comportamiento:* no sincroniza ni usa 4821 como número: pregunta [Sincronizar #4821 desde ADO] / [Crear una historia nueva (el número es solo contexto)] y sigue la respuesta.
...
- *Comportamiento:* El agente lee el `README.md` completo del SRS → agrupa los `FR-XXX` en tres historias candidatas por pantalla/actor (carga y consulta de facturas — pantalla 1; notificaciones — capacidad transversal a la anterior; gestión de usuarios — pantalla 2, sin dependencias) → detecta que «notificaciones» depende de que exista «carga y consulta» → presenta el agrupamiento y el orden al usuario, quien lo confirma → crea las tres US en ese orden, heredando de cada `FR-XXX` su criterio de verificación (sección 12 del SRS) como base del `AC-XXX`, los repositorios de la sección 4, y los wireframes aprobados de la sección 11 como Referencias (DoR **Referencias de UI** en `Cumple` de inmediato) → al terminar, actualiza la tabla **Historias de usuario derivadas** (sección 15) del SRS con las tres `US-XXX` y qué `FR-XXX` cubre cada una.
...
- Completar al sincronizar los criterios, dependencias o repositorios con lo que «probablemente» quiso decir el work item, o conceder `Ready` solo porque en el tracker está activo.
...
- Crear o editar documentos en `docs/architecture/` directamente desde este skill **salvo `wireframes/` y su fila de índice** (paso 3); la documentación técnica (`models/`, `flows/`, `diagrams/`, APIs) siempre se delega a `/design-define` vía subagente, y aquí solo se enlazan las referencias devueltas.
- Guardar wireframes en `assets/` de la US, nombrarlos sin `WF-XXX`, generar uno nuevo para una pantalla que ya tiene `WF-XXX` en la capability o que el SRS ya wireframeó, o crearlos sin su fila en el índice «Wireframes» del README de la capability.
- Presentar los wireframes uno por turno o preguntar aprobación pantalla por pantalla, en lugar de generar todo el lote y preguntar una sola vez.
...
- Terminar de crear las historias derivadas de un `SRS-XXX` sin actualizar su tabla de **Historias de usuario derivadas** (sección 15), dejando el SRS desactualizado sobre qué ya se descompuso.
- Ofrecer `/arch-init` sin verificar antes si el repositorio realmente carece del harness, o al revés, no ofrecerlo cuando un repositorio nuevo (que por definición no tiene `AGENTS.md`/`.sdd-devkit/settings.json`) lo necesita.
