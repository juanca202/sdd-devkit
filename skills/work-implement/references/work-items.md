...
| `Tipo` del WI | Rama |
| ------------- | ---- |
| `bug` | **No se implementa.** Es el registro de un hallazgo (que esta mal), no un plan. Parar y remitir: `work-research` «Analizar issue» sobre ese WI, o un `WI` de tipo `bug-fix` que lo cite; en un repo con `implementation.scope: tests`, informar que la correccion se planifica en el repositorio de la aplicacion. |
...
- **La rama de integracion la declara el repo, no el usuario.** Antes de tocar codigo, resolverla con [`${PLUGIN_ROOT}/references/git.md`](../../../references/git.md) — lectura obligatoria — y actuar segun lo que devuelva para la **rama actual**:
...
- **README de test cases:** si la carpeta `test-cases/` existe, **leer su `README.md`** (`<changesPath>/work-items/WI-XXX-[kebab-case]/test-cases/README.md`) para identificar que `TC-XXX` describen y cuales son **automatizables** (unit, integracion, e2e). Esta lectura alimenta el ciclo TDD del Paso 3 y las notas de cobertura en `progress.md`. Si el `README.md` no existe pero hay archivos `TC-XXX-*.md`, leer los propios test cases como fuente.
