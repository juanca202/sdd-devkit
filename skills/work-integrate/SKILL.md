...
| **Automatización de pruebas** | rama con prefijo `test/` + `FT-XXX`, `US-XXX` o `WI-XXX` | En la carpeta del artefacto padre: `docs/specs/features/FT-XXX-[slug]/progress.md`, o la de la US/WI correspondiente. **Repositorio de pruebas externo** (`implementation.target: external` en `.sdd-devkit/settings.json`): no hay `progress.md`; se lee `test-cases/automation.md` del padre — ver la nota de abajo | **todas** las unidades de esa ejecución (el `FT-XXX` completo, o cada `TC-XXX` del alcance) |
...
| **Estado de `progress.md`** | Leer el archivo en la ubicación correspondiente al tipo; si el trabajo ya está archivado, en su ruta bajo `docs/archive/`. En una rama `test/` de un repositorio de pruebas externo (`implementation.target: external`), el archivo es `test-cases/automation.md` del padre | Si no existe en ninguna de las dos: parar e informar; el merge requiere `progress.md` (o `automation.md`, en el caso externo) poblado |
