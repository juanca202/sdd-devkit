...
> **Alcance del submit:** El skill **cierra** localmente lo ya implementado. Verifica condiciones, **ofrece archivar el artefacto** (mover su carpeta a `docs/specs/archived/`, solo si el usuario lo confirma) y ejecuta `git merge --no-ff`. No hace push, no borra ramas, no crea MRs/PRs, no modifica el **contenido** de `progress.md`, y no resuelve conflictos —con la única salida declarada del `modify/delete` sobre los dos informes de `docs/audits/`, que se resuelve por el lado del borrado (ver [references/flows.md](references/flows.md)). Lo que no esté en `Done` bloquea el merge — el usuario decide cómo proceder, nunca se fuerza.
...
|------|--------------------------|---------------|------------------------------|
| **Historia de usuario** | `US-XXX` | `docs/specs/changes/user-stories/US-XXX-[nombre-corto]/progress.md` (por carpeta de la US) | **todas** las `TK-XXX` de la US |
| **Tarea de mantenimiento** | `WI-XXX` | `docs/specs/changes/work-items/WI-XXX-[kebab-case]/progress.md` (por carpeta del WI) | **todas** las unidades del `WI-XXX` en su propio `progress.md` |
| **Automatización de pruebas** | rama con prefijo `test/` + `FT-XXX`, `US-XXX` o `WI-XXX` | En la carpeta del artefacto padre: `docs/specs/current/FT-XXX-[slug]/progress.md`, o la de la US/WI correspondiente. **Repositorio solo de pruebas** (`implementation.scope: tests` en `.sdd-devkit/settings.json`): no hay `progress.md`; se lee `test-cases/automation.md` del padre — ver la nota de abajo | **todas** las unidades de esa ejecución (el `FT-XXX` completo, o cada `TC-XXX` del alcance) |
...
>
> **Ramas `test/` en un repositorio solo de pruebas** (`.sdd-devkit/settings.json` con `implementation.scope: tests`): ese repositorio **no tiene `progress.md`** y no se le exige. El archivo a leer es **`test-cases/automation.md`** en la carpeta del padre, y el estado de cada unidad es el **valor** de su línea `**Estado:**` (o `**Status:**`) bajo el encabezado `### <ID>: …` — `Pending`, `In Progress` o `Done`, siempre en inglés — porque ese archivo **no lleva marcas ocultas** `<!-- unit:… -->`. Comparar case-insensitive y sin espacios extra; cualquier valor distinto de `Done` bloquea el merge. Todo lo demás del flujo (puertas, rama base, merge, cierre) es idéntico; en el paso 11 el `Done` del trabajo se marca en la línea `**Estado:**` del encabezado de `automation.md`. Con `scope: code` o sin la clave, esta nota no aplica.
...
| Artefacto | Ruta |
|-----------|------|
| Carpeta / documento del trabajo | US: `docs/specs/changes/user-stories/US-XXX-[nombre-corto]/` · WI: `docs/specs/changes/work-items/WI-XXX-[kebab-case]/` · FT: `docs/specs/current/FT-XXX-[slug]/` |
...
| Destino del archivado (paso 11) | US: `docs/specs/archived/user-stories/US-XXX-[nombre-corto]/` · WI: `docs/specs/archived/work-items/WI-XXX-[kebab-case]/` · investigaciones sueltas huérfanas: `docs/specs/archived/research/RS-XXX-[slug]/` |
...
- La carpeta/documento del trabajo se deriva descontando el prefijo de rama y leyendo el identificador: `feature/US-042-exportacion-csv` → `docs/specs/changes/user-stories/US-042-exportacion-csv/`; `fix/WI-007-cache-ttl` → `docs/specs/changes/work-items/WI-007-cache-ttl/`; `test/FT-003-carga-masiva` → `docs/specs/current/FT-003-carga-masiva/` (cada uno con su propio `progress.md`).
...
| La carpeta del trabajo movida a `docs/specs/archived/` y el `progress.md` en `Done` | No viajan en el merge: se hacen **en la rama base, después del merge** (paso 11), en un commit de cierre propio. El `coverage.md` que la carpeta lleva dentro se mueve con ella. |
