...
- Hacer merge sin haber ejecutado `coverage-verify`, con veredicto `REJECTED`, o corriéndolo **antes** de `quality-check` (perdería la reutilización de `test-run.json`) — va después de `quality-check` y solo procede con `APPROVED` / `APPROVED_WITH_NOTES`.
