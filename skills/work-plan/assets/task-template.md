...
- **Documentación técnica:** {{enlace si aplica — DTOs, ER, flujos, endpoints — tal como lo devuelve `design-define`: `docs/architecture/[capability]/README.md#<id>` para APIs (ancla = id en minúsculas, `#api-004`) o `docs/architecture/[capability]/models/MD-XXX-{slug}.md` / `flows/FL-XXX-{slug}.md` / `diagrams/DG-XXX-{slug}.md` para modelos, flujos y diagramas; copiar la referencia tal cual, nunca recomponerla del título}}
