...
Descubrir Reglas de Negocio     ┘
    │
    ▼
Crear Features (FT-XXX)        ← <currentPath>/FT-XXX-{slug}/README.md
...
> `<currentPath>/FT-XXX-{slug}/` con su `README.md`.
...
**Entregable:** un `RS-XXX` en `<changesPath>/research/RS-XXX-{slug}/` con `README.md`
...
más un `FT-XXX` por feature aceptado en `<currentPath>/`.
...
4. **Separación de artefactos.** Los features y las pruebas resultantes registran
   ...
   `<currentPath>/` —junto a `<changesPath>/user-stories/` y `<changesPath>/work-items/`,
...
propio subárbol `<currentPath>/` (junto a `user-stories/` y `work-items/`):

```text
<proyecto>/<currentPath>/
└── FT-XXX-{slug}/
...
- `XXX` del RS: secuencial de tres dígitos sobre las carpetas `RS-XXX-*` de
  `<changesPath>/research/` **y de `<archivedPath>/research/`** (mayor + 1 entre las dos;
...
- `XXX` del FT: secuencial de tres dígitos sobre las carpetas `FT-XXX-*` de
  `<currentPath>/` (mayor + 1; `001` si no hay). Es independiente de la numeración
  de `<changesPath>/user-stories/` y `<changesPath>/work-items/`.
...
**aceptado** del mapa (Paso 1.9), crea una carpeta `<currentPath>/FT-XXX-{slug}/`
...
1. **Numeración.** Calcula el siguiente `FT-XXX` leyendo **solo** las carpetas
   `FT-XXX-*` de `<currentPath>/` (mayor + 1; `001` si no hay). Independiente de
...
2. **Destino:** los TCs se guardan **dentro de la misma carpeta del feature**, en
   `<currentPath>/FT-XXX-{slug}/test-cases/`, con su índice `README.md`, igual que
...
- Los features creados en `<currentPath>/` (IDs `FT-XXX` y títulos) y sus casos de
...
| Cada `FT-XXX` en `Ready` | `test-define` | Generar sus `TC-XXX` dentro de `<currentPath>/FT-XXX-{slug}/test-cases/` |
...
- Guardar los features o sus casos de prueba mezclados con los definidos por negocio
  (en `<changesPath>/user-stories/` o `<changesPath>/work-items/`) en vez de en su propio
  subárbol `<currentPath>/`.
