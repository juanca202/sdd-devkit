...
- `XXX`: secuencial de tres dígitos, calculado sobre las carpetas `RS-XXX-*`
  existentes en `<destino>/<changesPath>/research/` **y en
  `<destino>/<archivedPath>/research/`** (mayor + 1 entre las dos; `001` si no hay
...
considerando el número **más alto entre todos** los `<changesPath>/research/` —y sus
`<archivedPath>/research/`— de los destinos involucrados. En este documento "el
